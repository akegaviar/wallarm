...
прокси-сервер (:term:`reverse-proxy-term-ru`). В основу фильтрующего узла
...
* Принимает входящий HTTP-трафик, проверяет его соответствие определенным
  ...
  по принципу кольцевого буфера (:term:`circular-buffer-term-ru`). Это означает,
...
* Принимает из облака следующие данные:
    ...
    * правила фильтрации запросов: proton.db и LOM (:term:`lom-term-ru`).
    * правила обнаружения атак перебора (:term:`brute-force-term-ru`).
...
Для защиты от атаки типа MITM (:term:`mitm-term-ru`) используется несколько
средств защиты:

* сильное шифрование -- на стороне фильтрующего узла хранится
  Certificate Authority (:term:`ca-term-ru`), позволяющий однозначно идентифицировать
  облако.
...
* proton.db -- база данных, содержащая глобальные правила, применяемые для
  всех приложений.
* LOM -- :term:`lom-term-ru`. Правила безопасности для конкретного
